window.onload = () => {
    ...
    // Interface Elements
    const SUBDIV_TOGGLE_BUTTONS = document.getElementById("interface-subdiv-container");//Element which will hold an event listener, expanding each of the interface subdivs when the corresponding toggle button is clicked
    ...
    function changeColor(button) {
        button.addEventListener("click", (event) => {
            ...
            } else {//Since there is more than one hand, getting them all using an ID would not be possible
            ...
            if (elementToChange == "interface") {//When the user changes the interface color, the text for the color buttons and toggle buttons will be changed to that color as well
    ...
    Array.prototype.forEach.call(
        COLOR_BUTTONS, changeColor
    )

    SUBDIV_TOGGLE_BUTTONS.addEventListener("click", toggleColorPanels)
